...
app = FastAPI(title="IBM Infrastructure Inspect - YOLOv8 + Granite Diagnostic Core", version="10.0.0")
...
class YOLOCrackDetector:
  def __init__(self):
    ...
    if YOLO_AVAILABLE:
      try:
        weights_options = ["crack_yolo.pt", "best.pt", "yolov8n.pt"]
        selected_weights = "yolov8n.pt"
...
def read_status():
  return {
      "status": "healthy",
      "engine": "YOLO-V8 Diagnostics Core",
